...
if ($con) {
  ...
  if (isset($_GET["action"]) && $_GET["action"] == "update") {
    ...
    $item_discount = ucwords($_GET["discount"]);
    // $rate = ucwords($_GET["rate"]);
    // updateMedicineStock($id, $batch_id, $expiry_date, $quantity );
    updateMedicineStock($id, $batch_id, $quantity, $mrp, $item_discount);
  }
...
function showMedicinesStock1($id)
{
  require "db_connection.php";
  if ($con) {
    $seq_no = 0;
...
?>
  <tr>
    ...
    <td style="display: none;"><?php echo $row['GENERIC_NAME']; ?></td>
    <td><?php echo $row['BATCH_ID']; ?></td>
    <!-- <td style="display: none;"><?php echo $row['EXPIRY_DATE']; ?></td> -->
...
?>
  ...
  <tr>
    ...
    <td>
      <input type="number" class="form-control" value="<?php echo $row['DISCOUNT']; ?>" placeholder="DISCOUNT" id="item_discount" onkeyup="checkValue(this.value, 'item_discount_error');">
      <code class="text-danger small font-weight-bold float-right" id="item_discount_error" style="display: none;"></code>
    </td>
    <!--  <td>
...
function updateMedicineStock($id, $batch_id, $quantity, $mrp, $item_discount)
{
  require "db_connection.php";
  $query = "UPDATE medicines_stock SET BATCH_ID = '$batch_id', QUANTITY = $quantity, MRP = $mrp, DISCOUNT=$item_discount WHERE ID = $id";
  //  echo $query;
  $result = mysqli_query($con, $query);
  echo $result;
  if (!empty($result))
