<?php
  ...
  if($con) {
    ...
    if(isset($_GET["action"]) && $_GET["action"] == "update") {
      ...
      updateMedicine($id, $name, $packing, $suppliers_name);
    }
...
function showEditOptionsRow($seq_no, $row) {
  ...
  <tr>
    ...
    <td>
      <button  class="btn btn-success btn-sm" onclick="updateMedicine(<?php echo $row['ID']; ?>);">
...
function updateMedicine($id, $name, $packing, $suppliers_name) {
   
  require "db_connection.php";
  $query = "UPDATE medicines SET NAME = '$name', PACKING = '$packing',  SUPPLIER_NAME = '$suppliers_name' WHERE ID = $id";
  $result = mysqli_query($con, $query);
  if(!empty($result))
  $query = "UPDATE medicines_stock SET NAME = '$name' WHERE ID = $id";
  $result = mysqli_query($con, $query);
    showMedicines(0);
}
