...
function updateMedicineStock(id) {
  ...
  var rate = document.getElementById("mrp");
  var expiry_date = document.getElementById("mrp");
  ...
  //   expiry_date.focus();
    ...
    mrp.focus();
  else if (!checkValue(item_discount.value, "item_discount_error"))
    item_discount.focus();
  ...
  //   rate.focus();
    ...
    xhttp.open("GET", "php/manage_medicine_stock.php?action=update&id=" + id + "&batch_id=" + batch_id.value + "&quantity=" + quantity.value + "&mrp=" + mrp.value + "&discount=" + item_discount.value, true);
    xhttp.send();
  }
}
...
function searchItemStock(text, tag) {
  // if(tag == "NAME") {
