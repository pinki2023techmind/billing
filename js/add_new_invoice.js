...
function fillFields(medicine_name, id) {
  ...
  fill(medicine_name, 'discount_' + id, 'DISCOUNT');
  getTotal(id);
...
function getTotal(id) {
  ...
  for (var i = 1; i < row_count; i++) {
    ...
    tax = (Number.parseFloat(medicine_info[i].children[0].children[7].children[0].value));
    // alert(discount);
  ...
  // alert(total_amount - total_discount+"tax is"+ tax)
  total_tax = ((total_amount - total_discount) * tax) / 100
  var stax = tax_deduction;
  var ctax = tax_deduction;
...
function addInvoice() {
  ...
  var reffered = document.getElementById('reffered');
  ...
  if (!notNull(customers_name.value, "customer_name_error"))
  ...
  else if (reffered.value == "") {
    document.getElementById("reffered_error").style.display = "block";
    document.getElementById("reffered_error").innerHTML = "Enter Some Value!";
    reffered.focus();


  }
  else if (pa.value == "") {
    document.getElementById("paidAmount_error").style.display = "block";
    document.getElementById("paidAmount_error").innerHTML = "Enter Some Value!";
    pa.focus();
    ...
    for (var i = 1; i < row_count; i++) {
      ...
      //   medicine_name.focus();
        ...
        flag = true;
        //alert("row " + i + "perfect...");
        medicines[i - 1] = new MedicineInfo(medicine_name.value, batch_id.value, per.value, quantity.value, mrp.value, discount.value, total.value);
      }
    ...
    for (var i = 0; i < row_count - 1; i++) {
      ...
      addSale(customers_name.value, customers_contact_number.value, invoice_number.value, medicines[i].name, medicines[i].batch_id, medicines[i].expiry_date, medicines[i].quantity, medicines[i].mrp, medicines[i].discount, medicines[i].total, reffered.value);
    }

    addNewInvoice(invoice_number.value, stax.value, ctax.value, customers_name.value, customers_contact_number.value, invoice_date.value, total_amount.value, total_discount.value, net_total.value, total_tax.value, reffered.value);
...
function addSale(customers_name, customers_contact_number, invoice_number, medicine_name, batch_id, expiry_date, quantity, mrp, discount, total, reffered) {
  var xhttp = new XMLHttpRequest();
  xhttp.onreadystatechange = function () {
    if (xhttp.readyState = 4 && xhttp.status == 200)
      xhttp.responseText;
  ...
  xhttp.open("GET", "php/add_new_invoice.php?action=add_sale&customers_name=" + customers_name + "&customers_contact_number=" + customers_contact_number + "&invoice_number=" + invoice_number + "&medicine_name=" + medicine_name + "&batch_id=" + batch_id + "&expiry_date=" + expiry_date + "&quantity=" + quantity + "&mrp=" + mrp + "&discount=" + discount + "&total=" + total + "&reffered=" + reffered, true);
  xhttp.send();
}

function addNewInvoice(invoice_number, stax, ctax, customers_name, customers_contact_number, invoice_date, total_amount, total_discount, net_total, total_tax, reffered) {
  ...
  xhttp.open("GET", "php/add_new_invoice.php?action=add_new_invoice&stax=" + stax + "&ctax=" + ctax + "&customers_name=" + customers_name + "&customers_contact_number=" + customers_contact_number + "&invoice_date=" + invoice_date + "&total_amount=" + total_amount + "&total_discount=" + total_discount + "&net_total=" + net_total + "&total_tax=" + total_tax + "&reffered=" + reffered + "&invoice_number=" + invoice_number, true);
  xhttp.send();
}
